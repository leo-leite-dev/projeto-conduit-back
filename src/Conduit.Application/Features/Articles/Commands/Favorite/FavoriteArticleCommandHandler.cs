...
public sealed class FavoriteArticleCommandHandler
    : IRequestHandler<FavoriteArticleCommand, Result<ArticleResult>>
{
    ...
    public FavoriteArticleCommandHandler(
    ...
    )
    {
        ...
        var alreadyFavorited = await _favoriteRepository.ExistsAsync(article.Id, profile.Id, ct);

        if (!alreadyFavorited)
        {
            var favorite = ArticleFavorite.Create(profile, article);
